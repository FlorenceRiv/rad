...
elseif ((interaction isa Compton) && (interaction.model == "impulse_approximation") && (type == "S")) 
    ...
    for i in 1:Nz # for every element
        ...
        t_elem_start = time_ns()
        #println("\t Element ", i, " out of ", Nz, " , Z=", Z[i])

        _,Zi,Ui,_,_,_ = electron_subshells(Z[i],false) # electrons not free
        ...
        for gf in 1:Ng # Iterate through each final energy group
            ...
            t_group_start = time_ns()

            # Final energy group
            Ef⁻ = Eout[gf]; Ef⁺ = Eout[gf+1]
            Ef⁻,Ef⁺,isSkip = bounds_dispatch(interaction,Ef⁻,Ef⁺,Ei,gi,gf,type,minimum_binding_energy,Ec,incoming_particle)
            if isSkip
                #t_group = (time_ns() - t_group_start) / 1e9
                #println("\t\t Group ", gf, " out of ", Ng, " -- energy range [", Eout[gf], ", ", Eout[gf+1], "] mₑc² -- skip. -- total time: ", round(t_group, digits=4), " s")
                continue end
            ...
            if Ef⁻ < Ef⁺ 
                #t_group = (time_ns() - t_group_start) / 1e9
                #println("\t\t Group ", gf, " out of ", Ng, " -- energy range [", Eout[gf], ", ", Eout[gf+1], "] mₑc² -- skip. -- total time: ", round(t_group, digits=4), " s")
                continue end # if the upper bound is now lower than the lower bound skip this group
            ...
            if Eout[gf] != Ef⁻ || Eout[gf+1] != Ef⁺
                #println("\t\t Group ", gf, " out of ", Ng, " -- energy range [", Eout[gf], ", ", Eout[gf+1], "] mₑc² -- cropped range [", Ef⁻, ", ", Ef⁺, "] mₑc² -- total time: ", round(t_group, digits=4), " s")
            else
                #println("\t\t Group ", gf, " out of ", Ng, " -- energy range [", Eout[gf], ", ", Eout[gf+1], "] mₑc² -- total time: ", round(t_group, digits=4), " s")
            end    

        end

        t_elem = (time_ns() - t_elem_start) / 1e9
        #println("\t Element ", i, " total time: ", round(t_elem, digits=4), " s")

    end
    t_feed = (time_ns() - t_feed_start) / 1e9
    #println("feed() total time: ", round(t_feed, digits=4), " s")
end
return 𝓕, 𝓕ₑ
end
