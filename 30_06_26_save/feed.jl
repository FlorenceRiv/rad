    ...
    for i in 1:Nz
        ...
        for (Zi_s, Ui_s, Jio_s) in zip(Zi, Ui, Jio) # go over subshells
            ...
            for gf in 1:Ng
                ...
                Ef⁻ = Eout[gf]; Ef⁺ = Eout[gf+1]
                Ef⁻, Ef⁺, isSkip = bounds_dispatch(interaction, Ef⁻, Ef⁺, Ei, gi, gf, type, Ui_s, Ec, incoming_particle) # crops Ef- for RIA
                if isSkip; continue; end
                if Ef⁻ <= Ef⁺; continue; end

                # avoid near 0 integrals test
                #if (Ef⁻ - Ef⁺) < 1e-12
                #    println("skipping near 0 intgral, Ef⁻ - Ef⁺ = ", Ef⁻ - Ef⁺, " < 1e-12")
                #    continue
                #end
                ...
                group_moments, _ = quadgk(Ef⁺, Ef⁻) do Ef
                    ...
                    return [ddcs_ria_subshell(Ei, Ef, Zi_s, Ui_s, Jio_s, μ) * Radiant.legendre_polynomials(l, μ) for l in 0:L]
                    end
    ...
    for i in 1:Nz
        ...
        for (Zi_s, Ui_s, Jio_s) in zip(Zi, Ui, Jio) # go over subshells
            ...
            for gf in 1:Ng 
                ...
                Ef⁻, Ef⁺, isSkip = bounds_dispatch(interaction, Ef⁻, Ef⁺, Ei, gi, gf, type, Ui_s, Ec, incoming_particle) # crops Ef⁻ at min(Ef⁻, Ei-Ui) for RIA
                if isSkip; continue; end
                if Ef⁻ <= Ef⁺; continue; end
                ...
                group_moments, _ = quadgk(Ef⁺, Ef⁻) do Ef
                    ...
                    # integrate legendre moment (ddcs over mu) for each l :
                    dcs_moment_array, _ = quadgk(-1.0, 1.0) do μ
                    return [ddcs_ria_subshell(Ei, Efγ, Zi_s, Ui_s, Jio_s, μ) * Radiant.legendre_polynomials(l, μ) for l in 0:L]
                    end
