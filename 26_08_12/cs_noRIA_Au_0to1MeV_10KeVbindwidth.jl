...
cs.set_interactions([
                    Elastic_Collision(),Inelastic_Collision(),Bremsstrahlung(),Annihilation(),
        compton_interaction
                    ,Rayleigh(),Photoelectric(),Pair_Production(),Relaxation()
]) 
cs.set_legendre_order(7)
cs.build()
timestamp = replace(string(now()), r"[:\.T]" => "_")
JLD2.save("cs_noRIA_Au_0to1MeV_10KeVbinWidth_$(timestamp).jld2", Dict("xs_full" => cs))
