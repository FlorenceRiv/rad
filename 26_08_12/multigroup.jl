...
for gi in range(1,Ngi)
    ...
    for ni in range(1,Np)
        ...
        ################# for compton ria: replaced tcs calculation with sum of l=0 moments and 0 to cutoff energy integration addition:
        if type ∈ ["S","A"] && scattering_model != "FP"
            Σtᵢ = 0.0

            if !((interaction isa Compton) && (interaction.model == "impulse_approximation") && (type == "S"))
            ...
            # sum l=0 moments for above cutoff energy contribution to total cs

                Σtᵢ = w[ni]/2 * sum(𝓕[:, 1]) * N_density # no need to multiply by, atz[i] feed did it already

            # 0 to cutoff energy contribution to total cs

                Ef_cutoff = E_out[end] # lowest boundary of outgoing energy grid
                ...
                for i in 1:Nz
                    ...
                    for (Zi_s, Ui_s, Jio_s) in zip(Zi, Ui, Jio) # go over subshells
                        if Ei <= Ui_s; continue; end # skip if incoming energy smaller than ionization energy

                        Ef⁻ = min(Ef_cutoff, Ei - Ui_s)
                        if Ef⁻ <= 0.0 continue end

                        σ_below, _ = quadgk(0, Ef⁻) do Ef
                            mu_integral, _ = quadgk(-1.0, 1.0) do μ
                            return ddcs_ria_subshell(Ei, Ef, Zi_s, Ui_s, Jio_s, μ)
                            end
                            return mu_integral * 2π # integrate over phi
                        end

                        Σtᵢ += w[ni]/2 * σ_below * N_density * atz[i]
